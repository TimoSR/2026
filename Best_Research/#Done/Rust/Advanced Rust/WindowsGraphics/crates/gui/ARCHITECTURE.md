...
## Discoverable UI Macros

`gui::ui` is the curated public vocabulary for building UI trees. Typing
`ui::` should expose component macros, not implementation constructors,
layout structs, renderer modules, or styling helpers.

```rust
use gui::ui;

let settings_panel = ui::panel! {
    width: 320;
    padding: 24;
...
The macro options are local to the component being built. `width`, `height`,
`margin`, `padding`, and `weight` are not separate entries in the `ui::`
namespace.

The public `ui` module is the outer macro module. Its public surface re-exports
only macro names such as `panel`, `text`, `button`, `list`, `row`, and
`column`. The constructors those macros require live in `ui::__private`, a
doc-hidden bridge required by Rust macro visibility rules. Product code should
not call that bridge.

`UserInterfaceNode` is an opaque public node type. It exists so `gui::new(...)`
can accept the result of a UI macro without exposing the retained
`UserInterfaceBox` implementation.
