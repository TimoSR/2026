...
impl Force {
    ...
    pub const fn display_newtons(self) -> ForceDisplay { self.display_as(ForceUnit::Newtons) }

    pub const fn display_millinewtons(self) -> ForceDisplay { self.display_as(ForceUnit::Millinewtons) }

    pub const fn display_kilonewtons(self) -> ForceDisplay { self.display_as(ForceUnit::Kilonewtons) }

    pub const fn display_newtons_precision(self, precision: usize) -> ForceDisplay { self.display_as_precision(ForceUnit::Newtons, precision) }
...
impl fmt::Display for Force {
    fn fmt<'formatter>(&self, formatter: &mut fmt::Formatter<'formatter>) -> fmt::Result { self.display_newtons().fmt(formatter) }
}
