...
use super::quantity::VelocityUnit;
...
impl Velocity {
    ...
    pub const fn display_meters_per_second(self) -> VelocityDisplay { self.display_as(VelocityUnit::MetersPerSecond) }

    pub const fn display_kilometers_per_hour(self) -> VelocityDisplay { self.display_as(VelocityUnit::KilometersPerHour) }
...
impl fmt::Display for Velocity {
    fn fmt<'formatter>(&self, formatter: &mut fmt::Formatter<'formatter>) -> fmt::Result { self.display_meters_per_second().fmt(formatter) }
}
