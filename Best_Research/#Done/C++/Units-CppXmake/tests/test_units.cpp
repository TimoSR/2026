...
public:
    ...
    void check(
        ...
        failedChecks_ += 1;
        output_ << "    FAIL " << location.file_name() << ':' << location.line()
                << " `" << expression << "`\n";
    }
...
#define EXPECT(testContext, expression) \
    (testContext).check((expression), #expression, std::source_location::current())
...
void testEquationRulesIgnoreInputScale(TestContext& testContext) {
    ...
    EXPECT(
        testContext,
        approximatelyEqual(acceleration.asMetersPerSecondSquared(), (100.0 / 9.58) / 9.58, 1e-12)
    );
...
void testStdFormatSupport(TestContext& testContext) {
    ...
    EXPECT(
        testContext,
        std::format("{}", velocity.displayAsPrecision(units::VelocityUnit::KilometersPerHour, 2)) == "37.58 km/h"
    );
...
int main() {
    ...
    std::cout << "Checks: " << testContext.checksRun() - testContext.failedChecks()
              << " passed, " << testContext.failedChecks() << " failed\n";
