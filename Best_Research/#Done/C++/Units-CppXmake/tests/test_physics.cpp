...
#define EXPECT(testContext, expression) (testContext).check((expression), #expression, std::source_location::current())
    ...
    {
        EXPECT(testContext, Physics::length::meters(1) == Physics::length::centimeters(100));
        EXPECT(testContext, Physics::length::centimeters(100) == Physics::length::millimeters(1'000));
        EXPECT(testContext, Physics::time::seconds(1) == Physics::time::milliseconds(1'000));
        EXPECT(testContext, Physics::time::seconds(1) == Physics::time::microseconds(1'000'000));
        EXPECT(testContext, Physics::mass::kilogram(1) == Physics::mass::grams(1'000));
        EXPECT(testContext, Physics::mass::kilogram(1) == Physics::mass::milligrams(1'000'000));
    }
    ...
    {
        auto zeroTime = Physics::time::seconds(0);
        auto distance = Physics::length::meters(10);
        auto force = Physics::force::newtons(10);
        auto zeroMass = Physics::mass::kilograms(0);

        EXPECT(testContext, !distance.checkedDivTime(zeroTime).has_value());
        EXPECT(testContext, !force.checkedDivMass(zeroMass).has_value());
        EXPECT(testContext, distance.checkedDivTime(Physics::time::seconds(2)).has_value());
    }
    ...
    void testStdFormatSupport(TestContext& testContext)
    {
        ...
        EXPECT(testContext, std::format("{}", distance.displayAs(Physics::length::LengthUnit::Meters)) == "100 m");
        EXPECT(testContext, std::format("{}", velocity.displayAsPrecision(Physics::velocity::VelocityUnit::KilometersPerHour, 2)) == "37.58 km/h");
