...
impl<'a> RustCodeGenerator<'a> {
    pub fn new(flags: &'a FeatureFlags, context: &'a SemanticContext) -> Self {
    ...
    pub fn emit_item_into(&self, item: &TopLevelItem, output: &mut String) -> Result<()> {
        match item {
            TopLevelItem::Raw(raw) => output.push_str(&self.rewrite_statement_sugar(raw)?),
    ...
    fn emit_trait_into(&self, declaration: &TypeDeclaration, output: &mut String) -> Result<()> {
        ...
        let body = self.rewrite_statement_sugar(&rewrite_this_keyword(declaration.body.trim(), self.flags.this_receiver))?;
    ...
    fn normalize_field(&self, field: &FieldDeclaration) -> Result<String> {
        ...
        return Ok(self.rewrite_statement_sugar(&rewrite_this_keyword(&normalized, self.flags.this_receiver))?);
    }
    ...
    fn normalize_inherent_method(&self, method: &MethodDeclaration) -> Result<String> {
        ...
        return self.rewrite_statement_sugar(&rewrite_this_keyword(&with_pub, self.flags.this_receiver));
    }
    ...
    fn normalize_trait_impl_method(&self, method: &MethodDeclaration) -> Result<String> {
        ...
        return self.rewrite_statement_sugar(&rewrite_this_keyword(without_visibility, self.flags.this_receiver));
    }

    fn rewrite_statement_sugar(&self, source: &str) -> Result<String> {
        let with_csharp_declarations = rewrite_csharp_variable_declarations(source, self.context, self.flags)?;
        let with_new_expressions = rewrite_new_expressions(&with_csharp_declarations, self.context, self.flags)?;
        return rewrite_stack_heap_initializers(&with_new_expressions, self.context, self.flags);
    }
}
