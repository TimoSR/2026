...
#[cfg(test)]
mod guide_attribute_tests {
    use super::*;
    use crate::config::RustPlusConfig;

    fn compile_guide_snippet(source: &str) -> String {
        return Transpiler::new(source.to_string()).transpile().expect("guide snippet should compile");
    }

    #[test]
    fn guide_attributes_written_on_classes_fields_and_methods_are_preserved() {
...
"#;
        ...
        assert!(generated_rust.contains("#[derive(Debug, Clone, PartialEq, Eq)]\npub struct Article"));
        assert!(generated_rust.contains("    #[doc = \"headline\"]\n    pub headline: String,"));
        assert!(generated_rust.contains("    #[inline]\n    pub fn new(headline: &str) -> Self"));
    }

    #[test]
    fn guide_attributes_can_be_disabled_for_projects_that_want_plain_rustplus() {
        let rust_plus_source = r#"
#[derive(Debug)]
class Article
{
...
"#;
        ...
        let error = Transpiler::with_config(rust_plus_source.to_string(), config)
            .transpile()
