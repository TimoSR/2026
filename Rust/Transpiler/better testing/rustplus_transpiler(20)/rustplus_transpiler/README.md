...
The project now includes [CODE_TOUR.md](CODE_TOUR.md), which explains the codebase in product language: source documents, project symbols, feature pipeline, generated Rust, and sibling file builds. Start there before reading parser/codegen internals.

## Tests as documentation

Rust Plus tests are written as guide snippets. A test should explain how a user writes `.rp` and what Rust is generated.

Preferred test names start with `guide_`:

```text
guide_write_a_class_and_get_a_struct_with_private_fields_by_default
guide_create_a_boxed_trait_object_with_heap_initializer
guide_keep_rust_attributes_without_teaching_the_transpiler_each_attribute
```

The full testing style guide is in [TESTING.md](TESTING.md).
