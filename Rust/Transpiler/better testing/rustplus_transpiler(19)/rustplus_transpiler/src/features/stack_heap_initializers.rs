//! `Type::Stack(...)` and `Type::Heap(...)` support.
//!
//! Product meaning:
//!
//! - `Account::Stack(args)` means “construct an owned `Account` value”.
//! - `Account::Heap(args)` means “construct a boxed `Account` value”.
//! - `let account: IAccount = Account::Heap(args)` means “box the concrete
//!   account as a dynamic trait object”.
//!
//! The feature has three readable jobs:
//!
//! 1. find initializer calls outside comments and strings
//! 2. validate that the target type can be constructed
//! 3. lower the call into ordinary Rust construction
//!
//! This module is still intentionally conservative string rewriting. It does not
//! try to parse all Rust expressions.
...
/// Rewrite all `Type::Stack(...)` / `Type::Heap(...)` usages in a source fragment.
///
/// This function is called after class/interface lowering, so it only deals with
/// statement/expression sugar and leaves unrelated Rust unchanged.
...
/// Validate one initializer call before lowering.
///
/// Validation stays here instead of inside codegen so disabled-feature errors and
/// invalid lowercase calls produce Rust Plus diagnostics before rustc sees them.
...
/// Lower stack construction to ordinary owned-value Rust construction.
...
/// Lower heap construction to `Box::new(...)`. Interface binding is handled by
/// the left-hand type annotation rewrite.
...
/// Insert small ownership conversions when the constructor signature requires
/// them, for example `"id"` -> `"id".to_string()` for a `String` parameter.
...
#[cfg(test)]
mod guide_stack_heap_initializer_tests {
    use super::*;
    use crate::ast::ParameterDeclaration;
    use std::collections::{HashMap, HashSet};

    fn project_where_article_can_be_created() -> SemanticContext {
    ...
    fn project_where_account_can_be_used_through_iaccount() -> SemanticContext {
        let mut project = project_where_article_can_be_created();
        project.class_names.insert(String::from("Account"));
        project.constructor_parameters.insert(
            String::from("Account"),
    ...
    fn lower_rust_plus_line(source: &str, project: &SemanticContext) -> String {
        let flags = FeatureFlags::default();
        return rewrite_stack_heap_initializers(source, project, &flags).unwrap();
    }

    fn guide_example(input: &str, output: &str, project: &SemanticContext) {
        let generated_rust = lower_rust_plus_line(input, project);
        assert_eq!(generated_rust, output);
    }

    #[test]
    fn guide_stack_creates_a_directly_owned_value() {
        let project = project_where_article_can_be_created();
    ...
    #[test]
    fn guide_heap_creates_a_boxed_value() {
        let project = project_where_article_can_be_created();
    ...
    #[test]
    fn guide_heap_can_fill_a_trait_typed_binding() {
        let project = project_where_account_can_be_used_through_iaccount();
    ...
    #[test]
    fn guide_stack_without_arguments_uses_default() {
        let project = project_where_article_can_be_created();

        guide_example(
            "    let article = Article::Stack();\n",
            "    let article: Article = Article::default();\n",
            &project,
        );
    }

    #[test]
    fn guide_qualified_type_paths_are_preserved() {
        let project = project_where_article_can_be_created();

        guide_example(
            "    let boxed_article = Domain::Article::Heap(\"My own new!\", \"Lol!\", \"stuff\");\n",
