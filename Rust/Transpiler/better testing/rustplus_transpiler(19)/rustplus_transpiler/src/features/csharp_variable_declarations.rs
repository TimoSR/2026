...
#[cfg(test)]
mod legacy_guide_csharp_declaration_tests {
    use super::*;
    use crate::ast::ParameterDeclaration;
    use std::collections::{HashMap, HashSet};

    fn legacy_csharp_declaration_flags() -> FeatureFlags {
    ...
    fn project_where_account_implements_iaccount() -> SemanticContext {
    ...
    fn legacy_example(input: &str, expected: &str) {
        let flags = legacy_csharp_declaration_flags();
        let project = project_where_account_implements_iaccount();
        let generated_rust = rewrite_csharp_variable_declarations(input, &project, &flags).unwrap();

        assert_eq!(generated_rust, expected);
    }

    #[test]
    fn legacy_guide_interface_variable_becomes_boxed_trait_object() {
        legacy_example(
            "    IAccount account = new Account();\n",
            "    let mut account: Box<dyn IAccount> = Box::new(Account::new());\n",
        );
    }

    #[test]
    fn legacy_guide_concrete_variable_becomes_typed_let_binding() {
        legacy_example(
    ...
    #[test]
    fn legacy_guide_string_literals_are_converted_for_string_constructor_parameters() {
        legacy_example(
    ...
    #[test]
    fn legacy_guide_comments_and_strings_are_not_rewritten() {
        legacy_example(
            "    // IAccount account = new Account();\n    println!(\"new Account()\");\n",
            "    // IAccount account = new Account();\n    println!(\"new Account()\");\n",
        );
