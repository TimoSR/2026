...
## Human-readable code tour

See [CODE_TOUR.md](CODE_TOUR.md) for the non-compiler-expert map of the project.

## Readability refactor

The codebase now exposes human-readable concepts before lower-level implementation details:

```text
RustPlusCompiler       public API for compiling source text, documents, or projects
SourceDocument         one user-authored .rp file
ProjectSymbols         project-wide facts: classes, interfaces, constructors
FeaturePipeline        ordered feature validation
SiblingFileBuild       Cargo build.rs workflow for .rp -> sibling .rs
GeneratedRust          generated .rs text and source metadata
```

`SemanticContext` and `FeatureRegistry` remain as compatibility aliases, but new code should prefer `ProjectSymbols` and `FeaturePipeline`.
